/*
	jQuery floating header plugin v1.4.1
...
(function($){
	...
	function createFloater(target, template, config) {
		// (dpe01) target.width(template.width());
        
        // Clone colgroup too (npe01 start)
        if (template.children('colgroup').length > 0) {
            var colgroup = jQuery('<colgroup/>');
            template.children('colgroup').eq(0).children().clone().appendTo(colgroup);
		...
		items.each(function() {
			...
			row.children().each(function() {
                ...
                //var i = cell.innerWidth();
                //var o = cell.outerWidth();
                var w1 = cell.width();
				floatCell.width(Math.round(cell.width()));
                var w2 = floatCell.width();
