...
RESIZE_TO1 = 2400
RESIZE_TO2 = 1600
DEFAULT_IMAGE_BW_THRESHOLD = 240
TABLE_LINE_BLACK_THRESHOLD = 175
BACKGROUND_BLACK_THRESHOLD = 100
MIN_LINE_HOR = .6
MIN_LINE_VER = .6
...
class ImageProcessor():
    def __init__(self, image, *,
                 show_borders=True,
                 black_threshold=DEFAULT_IMAGE_BW_THRESHOLD,
        ...
        # Конвертим в ЧБ numpy-массив
        if isinstance(image, np.ndarray) and image.ndim == 3:
            self.np_orig_image = cv2.cvtColor(self.np_orig_image / 255, cv2.COLOR_BGR2GRAY)
        ...
        # Создаём копию, над которой мы будем всячески издеваться
        self.gray_np_image = self.np_orig_image.copy()
        # Сохраняем размеры
        self.H, self.W, *_ = self.gray_np_image.shape
        logging.info((self.H, self.W))



        # Выравниваем
        self.gray_np_image = _align_image(self.gray_np_image)
        # Максимизируем контраст и чёткость для экрана
        self.np_image_for_screen = _add_contast(self.gray_np_image)
        # Немного размываем, чтобы смыть тонкие шумы
        self.gray_np_image = _blur_image(self.gray_np_image)
        # Удаляем мусор
        self.gray_np_image = _remove_background(self.gray_np_image)
    ...
    def find_lines_coords(self):
        ...
        self.image_without_lines = _remove_table_lines_on_image(self.bitmap_np_image, self.horizontal_lines,
                                                                self.vertical_lines)
        # Определяем координаты этих линий
        self.coords_of_horiz_lns, self.coords_of_vert_lns = _calcutale_lines_coords(self.horizontal_lines,
                                                                                    self.vertical_lines)

    def find_filled_cells(self):
        # Делаем первичную разметку
        self.filled_cells = ext_find_filled_cells(self.image_without_lines, self.coords_of_horiz_lns,
                                                  self.coords_of_vert_lns)
    ...
    def toggle_highlight_cell(self, x_vert_ind, y_horiz_ind, *, initial_mode=False):
        ...
        if x_vert_ind < 0 or x_vert_ind >= len(self.coords_of_vert_lns) - 1:
            logging.error('Cell x-index out of range: {}, cor.range: {}-{}'.format(x_vert_ind, 0,
                                                                                   len(self.coords_of_vert_lns) - 1))
            return
        if y_horiz_ind < 0 or y_horiz_ind >= len(self.coords_of_horiz_lns) - 1:
            logging.error('Cell y-index out of range: {}, cor.range: {}-{}'.format(y_horiz_ind, 0,
                                                                                   len(self.coords_of_horiz_lns) - 1))
            return
        ...
        if not initial_mode ^ self.filled_cells[y_horiz_ind][x_vert_ind]:  # initial_mode инвертирует логику
            pre = (1 - ALPHA) * self.bgr_img_with_highlights[h1:h2, v1:v2, :] + ALPHA * CELL_FILL_COLOR
            self.bgr_img_with_highlights[h1:h2, v1:v2, :] = np.minimum(self.bgr_img_with_highlights[h1:h2, v1:v2, :],
                                                                       pre)
        elif not initial_mode and self.filled_cells[y_horiz_ind][x_vert_ind]:
            self.bgr_img_with_highlights[h1:h2, v1:v2, :] = self.bgr_img_orig[h1:h2, v1:v2, :]
        if not initial_mode:
    ...
    def initial_mark_filled_cells(self):
        ...
        # Делаем более контрастную версию для экрана
        self.bgr_img_with_highlights = cv2.cvtColor(self.np_image_for_screen, cv2.COLOR_GRAY2BGR)
        self.bgr_img_orig = self.bgr_img_with_highlights.copy()
        # Помечаем ячейки
        ...
        if self.BW and self.coords_of_horiz_lns and self.coords_of_vert_lns:
            ...
            for v_b in self.coords_of_vert_lns:
                ...
                self.bgr_img_with_highlights[top_v:bot_v, v1:v2, :] = \
                    .5 * self.bgr_img_with_highlights[top_v:bot_v, v1:v2, :] + .5 * BORDER_COLOR
            for h_b in self.coords_of_horiz_lns:
                h1, h2 = h_b - bw, h_b + bw
                self.bgr_img_with_highlights[h1:h2, left_h:right_h, :] = \
                    .5 * self.bgr_img_with_highlights[h1:h2, left_h:right_h, :] + .5 * BORDER_COLOR
    ...
    def coord_to_cell(self, x, y, w, h):
        ...
        if x_ind < 0 or y_ind < 0 or x_ind >= len(self.coords_of_vert_lns) - 1 or y_ind >= len(
                self.coords_of_horiz_lns) - 1:
            res = None
        else:
            res = [x_ind, y_ind]
...
def _remove_background(gray_np_image):
    """Удаляет мелкий сор из изображения"""
    # TODO Ну треш же...
    med = cv2.medianBlur(gray_np_image, 75)
    dif = cv2.add(gray_np_image, 255 - med)
    dif[dif < BACKGROUND_BLACK_THRESHOLD] = 0
    if DEBUG:
        cv2.imwrite("_med.png", med)
    return dif


def _add_contast(gray_np_image):
    """Заметно повышаем контрастность"""
    return ((gray_np_image.astype(np.float) / 16) ** 2).astype(np.uint8)
...
def _remove_table_lines_on_image(gray_np_image, horizontal_lines, vertical_lines):
    ...
    # TODO: Здесь мутные константы, которые я подбирал руками для наших кондуитов. Это — треш
    img_no_lines = ~cv2.adaptiveThreshold(img_no_lines, DEFAULT_IMAGE_BW_THRESHOLD, cv2.ADAPTIVE_THRESH_MEAN_C,
