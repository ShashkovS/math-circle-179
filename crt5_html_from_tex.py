...
def process_images(data, wrk):
    ...
    img_text = r'\\задача<img alt="\2" style="width:100%; max-width:300px;" src="i/{cl:02}-{lv}-IXX.png"/>\\кзадача'.format(cl=cur_les, lv=wrk['short_eng_level'])
    img_text_w = r'\задача<img alt="{alt}" style="width:100%; max-width:{wd}px;" src="i/{cl:02}-{lv}-IXX.png"/>\кзадача'.format(cl=cur_les, wd='{wd}', alt='{alt}', lv=wrk['short_eng_level'])
    img_text_lf_w = r'\задача<img alt="{alt}" style="float:left; width:100%; max-width:{wd}px;" src="i/{cl:02}-{lv}-IXX.png"/>\кзадача'.format(cl=cur_les, wd='{wd}', alt='{alt}', lv=wrk['short_eng_level'])
    pos = []
    for match in re.finditer(r'\\rightpicture\{(.*?)\}\{.*?\}\{(.*?)\}\{(.*?)\}', data, flags=re.DOTALL):
        wd = match.group(2).strip().replace('true', '')
        im_w = -1
        if wd.endswith('cm'):
            try: im_w = int(round(900/(20/int(wd[:-2]))))
            except:
                im_w = -1
        if wd.endswith('mm'):
            try:
