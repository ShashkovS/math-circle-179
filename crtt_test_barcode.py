...
def join_recognized_and_xlsx(recognized_pages, xlsx_data, stats):
    auds = sorted({x['Аудитория'] for x in xlsx_data if '100' <= x['Аудитория'] <= '333'})
    ...
    for item in xlsx_data:
        ...
        if 'Результат' in item and item['Результат']:
            ...
            for clmn, res in enumerate(item['Результат']):
                if res:
                    print('zzz', rown, clmn + col_mov + 4)
                    # sheet.write(rown, clmn + col_mov + 4, 1)
                    sheet.write(rown, clmn + col_mov + 4, 'x')
    workbook.save('results.xls')

if __name__ == '__main__':
    # os.chdir(r"Py_VMSH_2017")
    print(decode(Image.open("sum_page_0.png")))
    os.chdir(r"..")
