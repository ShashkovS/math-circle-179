...
for wrk in work:
    ...
    prev_name_teacher = os.path.join(DUMMY_FOLDER_PATH, wrk['dummy_tex_teacher_template'].format(cur_les=cur_les-1).replace('.tex', '.pdf')) ##
    prev_name_conduit = os.path.join(DUMMY_FOLDER_PATH, wrk['dummy_tex_conduit_template'].format(cur_les=cur_les-1).replace('.tex', '.pdf')) ##
